...
public class LoadBltin {
	...
	public int run() throws ClassNotFoundException, SQLException, IOException
	{
		...
		HTable dstTable=(HTable) TableFactory.getTable("Dsr2SiFaDoc");
		...
		String selectCmd="SELECT "+colStr+" FROM bltin where add_time>=@startDt and add_time<=@stopDt and rld_prn is not null and rld_prn!='' and id is not null"
				+"\n"+"union all"+"\n"+
				"SELECT "+colStr+" FROM bltin188 where add_time>=@startDt and add_time<=@stopDt and rld_prn is not null and rld_prn!='' and id is not null";		
		selectCmd=selectCmd.replace("@startDt", "'"+startDt+"'");
		selectCmd=selectCmd.replace("@stopDt", "'"+stopDt+"'");
